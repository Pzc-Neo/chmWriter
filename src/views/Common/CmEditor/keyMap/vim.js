...
;(function (mod) {
  ...
  'use strict'
  const { dictForVimFCommand } = require('./dictForVimFCommand')
  ...
  var defaultExCommandMap = [
    ...
    { name: 'quit', shortName: 'q' }, //neo_add
    { name: 'write_and_quit', shortName: 'wq' }, //neo_add
  ...
  var Vim = function () {
      ...
      for (var i = 0; i < ranges.length; i++) {
        ...
        if (range.empty()) {
          ...
          if (range.anchor.ch < lineLength) {
            ...
            result.push(
              cm.markText(
                Pos(range.anchor.line, lineLength - 1),
          ...
          return function () {
            return cm.operation(function () {
              ...
              try {
                ...
                // neo_add-start
                if (e.message.match('There is no line')) {
                  alert(
                    '编辑器渲染有点问题！【影响编辑，不影响保存】\n解决办法1：把本章节关闭再重新打开。\n解决办法2：撤销1步或者多步。'
                  )
                }
                // neo_add-end
                throw e
    ...
    var exCommands = {
      ...
      // neo_changed-start
      write: function (cm) {
        if (CodeMirror.commands.write) {
          // If a save command is defined, call it.
          CodeMirror.commands.write(cm)
        } else if (cm.write) {
      ...
      // neo_add-start
      quit: function (cm) {
        if (CodeMirror.commands.quit) {
          CodeMirror.commands.quit(cm)
        } else if (cm.quit) {
          cm.quit()
        }
      },
      write_and_quit: function (cm) {
        if (CodeMirror.commands.write_and_quit) {
          CodeMirror.commands.write_and_quit(cm)
        } else if (cm.write_and_quit) {
          cm.write_and_quit()
        }
      },
      // neo_add-end
